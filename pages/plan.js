...
function getPlanGateInfo(c) {
  ...
  const execSummary = window.EXECUTION_SYNC?.getOrBuild ? window.EXECUTION_SYNC.getOrBuild(window.ST) : (window.ST?.execution || null);
  const actionableRuntime = Number(execSummary?.bucketTotal || 0) > 0;
  ...
  const authorityTier = String(c?.authorityTier || c?.finalAuthorityStatus || c?.executionTier || c?.status || '').toUpperCase();
  const authorityDecision = String(c?.authorityDecision || (c?.executionActionable ? 'ALLOW' : '')).toUpperCase();
  const execCheck = {
    ok: !!c && authorityDecision !== 'REJECT' && ['READY','PLAYABLE','PROBE'].includes(authorityTier)
  };
  const setupKey = String(c?.setup || c?.structureTag || '').toLowerCase();
  const phaseAware = /phase c|phase-candidate|early phase d/.test(setupKey);
  const rrFloor = phaseAware ? 0.3 : 1.2;
  const rrBlocked = !rr1 || rr1 < rrFloor;
  const gateBlocked = actionableRuntime ? false : !!snap?.disableTrading;
  ...
  else if (rrBlocked) reason = `RR dưới ${rrFloor.toFixed(1)}${phaseAware ? ' (phase-aware)' : ''}`;
...
function renderPlan() {
  const execSummary = window.EXECUTION_SYNC?.getOrBuild ? window.EXECUTION_SYNC.getOrBuild(window.ST) : (window.ST?.execution || null);
  const coins = Array.isArray(execSummary?.coins) && execSummary.coins.length ? execSummary.coins : ST.coins;
  const suggested = execSummary?.bestCoin || (window.PRO_EDGE?.getSuggestedCoin ? window.PRO_EDGE.getSuggestedCoin() : null);
...
function useProEdgeSuggestion() {
  const snap = ST.scanMeta?.proEdge || null;
  const execSummary = window.EXECUTION_SYNC?.getOrBuild ? window.EXECUTION_SYNC.getOrBuild(window.ST) : (window.ST?.execution || null);
  const c = execSummary?.bestCoin || (window.PRO_EDGE?.getSuggestedCoin ? window.PRO_EDGE.getSuggestedCoin() : null);
  if (!c || (!execSummary?.bucketTotal && snap?.disableTrading)) { alert('Execution runtime chưa unlock coin trade được.'); return; }
  const setupKey = String(c.setup || c.structureTag || '').toLowerCase();
