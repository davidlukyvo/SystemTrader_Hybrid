...
function renderScorer() {
  ...
  <div class="card mb-20">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap">
      ${ST.coins.map(c => `
        <button class="btn btn-sm ${String(scorerState.coinId)===String(c.id)?'btn-primary':'btn-outline'}"
...
function renderScorerSummary() {
  ...
  return `
    <div class="score-total mb-16">
      ...
      <div class="score-number ${g.cls}">${total}</div>
      <div style="font-size:11px;color:var(--text-muted);margin:4px 0">/ 100</div>
      <div class="score-grade ${g.cls}">${g.grade}</div>
      <div class="score-desc">${g.desc}</div>
    </div>

    <!-- Breakdown -->
    <div class="card mb-16">
      <div class="card-title">Phân Tích Điểm</div>
      ${categories.map(([name, val, max]) => `
        <div style="margin-bottom:10px" id="bd_${name.toLowerCase().replace(' ', '')}">
    ...
    <!-- Decision Box -->
    <div style="margin-bottom:16px;padding:16px;border-radius:var(--radius-lg);
      background:${total>=85?'rgba(16,185,129,.08)':total>=75?'rgba(0,229,255,.06)':total>=65?'rgba(245,158,11,.06)':'rgba(239,68,68,.06)'};
      border:1px solid ${total>=85?'rgba(16,185,129,.25)':total>=75?'rgba(0,229,255,.2)':total>=65?'rgba(245,158,11,.2)':'rgba(239,68,68,.2)'}">
...
function updateScore(key, val, max) {
  ...
  // Update local elements (Label + Bar)
  const ptsEl = document.getElementById(`pts_${key}`);
  const barEl = document.getElementById(`bar_${key}`);
  if (ptsEl) ptsEl.textContent = `${val}/${max}`;
  if (barEl) barEl.style.width = (val/max*100) + '%';
  
  // Re-render only the summary panel to avoid losing focus on input
  const summaryEl = document.getElementById('scorerSummaryPanel');
  if (summaryEl) {
    summaryEl.innerHTML = renderScorerSummary();
  }
}
...
function loadScorerCoin(id) {
  ...
  // Fully re-render to update the coin list selection state
  renderScorer();
}
...
function saveScorerResult() {
  ...
  ST.save();
  renderScorer();
  showToast(`Điểm ${total}/100 cho ${c.symbol} đã lưu`, 'success');
}
...
function addToWatchlist() {
  ...
  ST.save();
  showToast(`Đã thêm ${c.symbol} vào tier "${tier}"`, 'success');
}
