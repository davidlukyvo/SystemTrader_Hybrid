...
- **Scheduler Jitter Mode**: `app.js → SMART_SCAN` can randomize automatic scan timing with `schedulerMode='jitter'`, `jitterMinMinutes`, `jitterMaxMinutes`, `minGapMinutes`, `nextAutoScanAt`, and run audit fields. This is ops/sampling only; it must not change Alpha Guard, thresholds, authority, capital, portfolio, Telegram eligibility, or `deployableTop3`.
