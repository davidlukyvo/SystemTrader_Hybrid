...
const SYMBOL_MAPPER = {
  ...
  async mapCoinToPair(coin, force = false) {
    ...
    if (window.CLEAN_UNIVERSE?.shouldExclude?.(coin)) return null;
...
async function retryFetchJson(url, options = {}, retries = 2, delayMs = 500) {
  ...
  for (let i = 0; i <= retries; i += 1) {
    ...
    try {
      ...
      lastErr = err;
      if (i < retries) await new Promise(r => setTimeout(r, delayMs * (i + 1) + Math.floor(Math.random() * 250)));
    } finally {
      clearTimeout(timer);
    }
  }
